...
def start_game():
    ...
    start_img_rect.center = (WINDOW_HEIGHT/2, WINDOW_WIDTH/2)
...
def game_loop():
    while True:
        ...
        while True:
            ...
            lvl_font = font.render('Уровень:'+str(LVL), True, GREEN)
            lvl_font_rect = score_font.get_rect()
            lvl_font_rect.center = (500, cactus_img_rect.bottom + score_font_rect.height/2)
            canvas.blit(score_font, score_font_rect)

            top_score_font = font.render('Топ Очков:'+str(Topscore), True, GREEN)
            top_score_font_rect = score_font.get_rect()
